...
while not done:
    ...
    #if(x != int(x_transport_cordinate) or y != int(y_transport_cordinate)):
    ...
    for el in working_response_server_var:
        curr_cord=el.split(' ')
        x_cord_array.append(int(curr_cord[0]))
        y_cord_array.append(int(curr_cord[1]))

    #the background
    screen.fill(WHITE)

    #draw the player
    for i in range(0, len(x_cord_array)):
        draw_player(screen, x_cord_array[i], x_cord_array[y])
